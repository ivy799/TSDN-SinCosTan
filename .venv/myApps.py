...
def create_pie_chart(feature_importances):
    fig, ax = plt.subplots()
    ax.pie(feature_importances, labels=feature_importances.index, autopct="%1.1f%%", startangle=90)
    ax.axis("equal")
    buf = BytesIO()
...
for island, coords in islands.items():
    island_data = data[data["Pulau"] == island]
    X_island = island_data.drop(columns=["Y", "Pulau"])
    y_island = island_data["Y"]
    xgb_model.fit(X_island, y_island)
    feature_importances = pd.Series(xgb_model.feature_importances_, index=X_island.columns).sort_values(ascending=False).head(5)
    pie_chart = create_pie_chart(feature_importances)
    popup_html = f"""
    <b>{island}</b><br>
    <img src="data:image/png;base64,{pie_chart}" alt="Pie chart">
    """
    folium.Marker(location=coords, popup=popup_html, tooltip=island).add_to(m)
...
for i, col_name in enumerate(X.columns):
    ...
    input_data[col_name] = col.number_input(
        label, min_value=0.0, max_value=100.0, step=0.1, key=f"input_{i}"
    )
...
if st.button("Prediksi"):
    ...
    if prediction[0] == 1:
        ...
        risk_factors_labels = [
            column_names_to_labels.get(factor, factor) for factor in risk_factors.index
        ]
        fig, ax = plt.subplots()
        ax.pie(
            risk_factors, labels=risk_factors_labels, autopct="%1.1f%%", startangle=90
        )
        ax.axis("equal")
        st.pyplot(fig)
